...
Public Class Manager_Report_Form
    ...
    Private Sub UpdateTotalSales()
        ...
        Try
            connection.Open()
            Dim query As String = "SELECT SUM(Amount) FROM payments"
            Dim cmd As New MySqlCommand(query, connection)
    ...
    Private Sub LoadSalesRecordsForDate(selectedDate As DateTime)
        ...
        Try
            ...
            ' Query to get sales records for the selected date including ProductName
            Dim query As String = "SELECT DATE(p.Date) AS Date, p.Amount, pr.ProductName " &
                              "FROM payments p " &
                              "JOIN tblproducts pr ON p.ProductName = pr.ProductName " &
                              "WHERE DATE(p.Date) = @SelectedDate"
            Dim cmd As New MySqlCommand(query, connection)
            cmd.Parameters.AddWithValue("@SelectedDate", selectedDate.Date) ' Pass the selected date as a parameter
            ...
            If reader.HasRows Then
                While reader.Read()
                    ...
                    ' Add the date, product name, and amount to DataGridView1
                    DataGridView1.Rows.Add(salesDate.ToString("yyyy-MM-dd"), productName, amount.ToString("N2"))
                End While
            Else
